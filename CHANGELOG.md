...
- **Sentry**, as a connector-specific boolean. Only `SENTRY_STATE_ENABLED` counts as watching: a parked, locked car reported `IDLE`, which is ambiguous, and under-counting Sentry is the recoverable mistake — anything downstream that prices an hour of it must not price a night that never happened. Widen once confirmed against a night with Sentry deliberately on.
